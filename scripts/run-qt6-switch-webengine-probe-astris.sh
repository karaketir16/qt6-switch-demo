...
NRO_PATH="${1:-${REPO_ROOT}/demo/webengine-app/qt6-switch-webengine-probe.nro}"
ASTRIS_APP="${ASTRIS_APP:-/Volumes/T7/Applications/Astris/Astris.app}"
ASTRIS_DATA="${ASTRIS_DATA:-/Volumes/T7/astrisData}"
TARGET_DIR="${ASTRIS_DATA}/homebrew/qt6-switch-webengine-probe"
LOG_DIR="${HOME}/Library/Containers/V380-Ori.Astris/Data/Library/Logs/Ryujinx"
GUEST_TRACE="${ASTRIS_DATA}/sdcard/qt6-switch-webengine-probe.log"
...
mkdir -p "${TARGET_DIR}"
cp -f "${NRO_PATH}" "${TARGET_DIR}/qt6-switch-webengine-probe.nro"
rm -f "${GUEST_TRACE}"

echo "Launching Astris with:"
echo "  ${TARGET_DIR}/qt6-switch-webengine-probe.nro"
echo
echo "Closing any previous Astris instance before starting the WebEngine demo."
echo

pkill -x Astris || true
sleep 1

open -a "${ASTRIS_APP}" "${TARGET_DIR}/qt6-switch-webengine-probe.nro"
handle_astris_restore_prompt
